...
const toggleBookmark = (ayat) => {
    const isExist = bookmarks.value.find((item) => item.surahId === Number(route.params.id) && item.ayatNumber === ayat.nomorAyat);

    if (isExist) {
        bookmarks.value = bookmarks.value.filter((item) => !(item.surahId === Number(route.params.id) && item.ayatNumber === ayat.nomorAyat));
    } else {
...
<template>
    <div v-if="errorMessage" class="flex flex-col items-center justify-center py-20 text-center">
        ...
        <button @click="router.go(0)" class="mt-5 bg-primary text-white px-5 py-2 rounded-xl cursor-pointer">Coba Lagi</button>
    </div>

    <div v-else class="relative">
        <div class="h-[80px] bg-primary w-full overflow-hidden relative z-0">
            <img src="../../../assets/alquranwithoutbg.png" class="absolute inset-0 w-full h-full object-cover opacity-30 scale-110" alt="alquran without background" />
        ...
        <div class="relative -mt-[75px] z-10 px-4 lg:px-28">
            <div class="bg-transparent rounded-t-3xl px-4 pt-4">
                <div class="flex justify-between items-center mb-6">
                    <div @click="router.push('/quran/surah')" class="flex items-center gap-2 cursor-pointer">
            ...
            <div class="flex items-center flex-col">
                <div class="bg-gray-400/20 w-full text-center flex flex-col items-center justify-start lg:justify-center py-8">
                    ...
                    <div class="flex gap-2 items-center mt-2">
                        <p class="border border-gray-500 px-3 py-1 rounded-lg text-gray-400"><i class="pi pi-book"></i> {{ surahStore.surahDetail.jumlahAyat }} Ayat</p>
                    ...
                    <div class="mt-4 flex gap-2 justify-start w-full px-10 lg:justify-center">
                        ...
                        <div ref="dropdownRefAyat" class="relative inline-block w-full lg:w-fit">
                            ...
                            >
                                <div v-for="ayat in surahStore.surahDetail.ayat" :key="ayat.nomorAyat" @click="scrollToAyat(ayat.nomorAyat)" class="py-2">
                                    {{ ayat.nomorAyat }}
                                </div>
                            </div>
                        </div>
                ...
                <div class="px-4 md:pr-8 md:pl-12 lg:px-28">
                    ...
                    <div v-for="ayat in surahStore.surahDetail.ayat" :key="ayat.ayat" class="py-2" :id="`ayat-${ayat.nomorAyat}`">
                        ...
                        <div class="flex items-center gap-3 mt-4">
                            <i class="pi pi-play-circle text-primary text-3xl cursor-pointer" @click="playAudio(ayat.audio?.['01'])"></i>
