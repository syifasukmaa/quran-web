...
const prayerIcons: Record<string, string> = {
  Subuh: 'pi-moon', Dzuhur: 'pi-sun', Ashar: 'pi-cloud-sun',
  Maghrib: 'pi-sunset', Isya: 'pi-star',
};
...
async function handleManualSubmit() {
  ...
  store.setLocation(
    `${selectedCity.value}, ${selectedProvince.value}`,
    'manual',
    selectedCity.value,
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      try {
        ...
        await provinceStore.fetchProvinces();
        const matchedProvince = provinceStore.provinces.find(p =>
        ...
        const matchedCity = provinceStore.cities.find(c =>
        ...
        await store.fetchPrayerTime({
          kabkota: matchedCity, provinsi: matchedProvince,
  ...
  <div class="bg-white dark:bg-slate-800 drop-shadow-md rounded-xl px-4 py-4 lg:mt-6 mt-12 ">
    ...
    <div class="flex items-center justify-between mb-3">
      ...
      <button @click="openManual"
        class="rounded-full px-3 py-1.5 text-sm border border-primary text-primary
               hover:bg-primary hover:text-white transition-all duration-200
               dark:border-teal-400 dark:text-teal-400 cursor-pointer">
    <i class="pi pi-pen-to-square mr-2 "></i>  Ubah Lokasi
      </button>
    </div>

    <div v-if="store.locationName" class="mb-3">
      <span class="inline-flex items-center gap-1.5 bg-green-50 border border-green-200
                   text-green-700 text-xs font-bold px-3 py-1 rounded-full
                   dark:bg-teal-900/30 dark:border-teal-700 dark:text-teal-400">
        <i :class="store.locationType === 'auto' ? 'pi pi-map-marker' : 'pi pi-map'" />
        {{ store.locationName }}
        <span v-if="store.locationType === 'auto'"
              class="bg-green-600 text-white text-[9px] px-1.5 py-0.5 rounded-full font-bold">
          AUTO
        </span>
      </span>
    </div>
    <div v-else class="mb-3">
      <span class="inline-flex items-center gap-1.5 bg-amber-50 border border-amber-300
                   text-amber-700 text-xs font-semibold px-3 py-1 rounded-full">
        Lokasi belum diatur
      </span>
    </div>


    <div v-if="!store.locationName && !showManual"
         class="bg-primary rounded-xl py-3 px-4 flex items-center justify-between gap-3 mb-3">
      ...
      <div class="flex flex-col gap-1.5 flex-shrink-0">
        <button @click="handleAutoLocation" :disabled="geoLoading"
          class="rounded-full px-3 py-1.5 text-xs font-bold bg-white text-primary
                 hover:bg-gray-100 transition-all disabled:opacity-60">
          {{ geoLoading ? 'Mendeteksi...' : 'Izinkan' }}
        </button>
        <button @click="showManual = true; provinceStore.fetchProvinces(); geoError=''; "
          class="rounded-full px-3 py-1.5 text-xs border border-white/60
    ...
    <div v-if="store.todaySchedule && !store.loading"
    ...
    <div v-if="prayers && nextPrayer && !store.loading"
         class="bg-gradient-to-r from-primary to-teal-400 rounded-xl p-4
                flex items-center justify-between mb-4">
      <div>
        <p class="text-xs text-white/70 font-bold uppercase tracking-wider mb-1">
          Sholat Berikutnya
        </p>
    ...
    <div v-if="prayers && !store.loading" class="grid grid-cols-5 gap-2 mb-2">
      <div v-for="(time, name) in prayers" :key="name"
        :class="[
        ...
        <p :class="['text-[12px] font-bold mb-0.5',
           name === nextPrayer?.name ? 'text-primary' : 'text-gray-500 dark:text-slate-400']">
          {{ name }}
        </p>
        <p :class="['text-xs font-black',
           name === nextPrayer?.name ? 'text-primary' : 'text-gray-800 dark:text-white']">
          {{ time }}
        </p>
      </div>
    </div>
    ...
    <div v-if="showManual"
         class="mt-4 border border-dashed border-gray-200 dark:border-slate-600
                rounded-xl p-4 bg-gray-50 dark:bg-slate-700/50">
      <p class="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-3">
        Atur Lokasi Manual
      </p>
      <div v-if="selectedProvince && !selectedCity" class="mb-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
       <p class="text-xs text-blue-600 dark:text-blue-400 font-semibold">
         Provinsi terdeteksi: <strong>{{ selectedProvince }}</strong><br>
      ...
      <div class="grid grid-cols-2 gap-3 mb-3">
        ...
        <div>
          ...
          <select v-model="selectedProvince"
            class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-500
                   bg-white dark:bg-slate-700 text-sm text-gray-800 dark:text-white
      ...
      <div class="flex gap-2">
        <button @click="handleManualSubmit" :disabled="!selectedCity || store.loading"
          class="flex-1 py-2.5 rounded-xl font-bold text-sm bg-primary text-white
        ...
        <button @click="showManual = false"
          class="px-4 py-2.5 rounded-xl font-bold text-sm border border-gray-200
                 text-gray-500 hover:bg-gray-100 dark:border-slate-500
                 dark:text-slate-400 dark:hover:bg-slate-700 transition-all">
          Batal
        </button>
      </div>
    </div>
